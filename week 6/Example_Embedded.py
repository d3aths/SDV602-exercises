...
def draw_figure(canvas, figure):
    ...
    #gets figure onto canvas, draws the figure,and returns it

def delete_figure_agg(figure_agg):
    figure_agg.get_tk_widget().forget()
    plt.close('all')
    #makes it forget what it was doing

def embedded_plt(fig_dict): #passes the dictionary here
    sg.theme('LightGreen')

    figure_w, figure_h = 650, 650
    # define the form layout
    listbox_values = list(fig_dict)  #produces the keys from dictionary as a list
